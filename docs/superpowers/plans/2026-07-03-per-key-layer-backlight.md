# Per-Key Layer-Specific Backlight Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Implement custom per-key layer-specific backlight highlights on the Sofle keyboard.

**Architecture:** We will create a custom C driver file `config/boards/shields/sofle/src/rgb_layer_colors.c` that registers as a listener for ZMK layer changes and underglow status. Upon shifting layers, the driver will calculate which keys are active/mapped, and update the WS2812 LED strip directly (keys at indices 6-35, with underglow LEDs at 0-5 remaining dark or set to the layer background). Unmapped keys are turned off entirely for complete isolation.

**Tech Stack:** ZMK, Zephyr RTOS LED Strip API, C, Devicetree.

## Global Constraints
* Code must compile cleanly under Zephyr 3.5+ / ZMK main branch.
* Do not stage or commit temporary files; only commit config and spec files.

---

### Task 1: Create Custom C Layer Color Listener

**Files:**
- Create: `config/boards/shields/sofle/src/rgb_layer_colors.c`
- Modify: `config/boards/shields/sofle/CMakeLists.txt`

**Interfaces:**
- Consumes: ZMK active layer state changes (`zmk_layer_state_changed` event).
- Produces: Layer tracking, active layer index evaluation, and LED updating triggering function.

- [ ] **Step 1: Write `config/boards/shields/sofle/src/rgb_layer_colors.c`**

Create the C file with the event listener, subscribing to ZMK's layer changes and defining color mappings:
...
static int rgb_layer_listener_cb(const zmk_event_t *eh) {
    ...
    if (ev) {
        update_layer_backlight(zmk_endpoints_selected());
    }
    return 0;
}
...
- [ ] **Step 2: Update `config/boards/shields/sofle/CMakeLists.txt`**

Append our C driver source file so it compiles cleanly:
```cmake
if(CONFIG_SOFLE_CUSTOM_DISPLAY)
  zephyr_library_named(sofle_custom_display)
  zephyr_library_include_directories(${CMAKE_SOURCE_DIR}/include)
  zephyr_library_sources(src/custom_status_screen.c)
endif()

if(CONFIG_ZMK_RGB_UNDERGLOW)
  zephyr_library_sources(src/rgb_layer_colors.c)
endif()
```
...
Run:
```bash
git add config/boards/shields/sofle/src/rgb_layer_colors.c config/boards/shields/sofle/CMakeLists.txt
git commit -m "feat: implement custom per-key layer-specific backlight event listener"
```
